...
any edges as having no resources to build the network. Therefore:
...
Also:
\begin{equation}\label{eq:eqf}
	p(k_i)=\frac{N_i}{N}=\frac{2C/C_2}{N_i k_i} \Rightarrow \frac{N_i^2}{N^2}=2\frac{C}{N.C_2}k_i^{-1} \Rightarrow p(k_i)\propto k_i^{-\frac{1}{2}}
\end{equation}

\noindent so that if $p(k_i)$ is observed only with respect to the degree, $\alpha=0.5$,
which is far from experimental evidence.
