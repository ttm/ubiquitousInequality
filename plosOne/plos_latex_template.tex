...
Section~\ref{sec:related} is dedicated to relating previous work with the content here presented.
...
but a truly unified framework for interpreting any power law, such as given in this document, was not found in the literature by the authors.
E.g. one may regard SOC (Self-Organized Criticality) as such a unified model, but SOC models presuppose that the system is a dynamical system~\cite{part}, as is the case for other models~\cite{slanina}.
More relevant, most models are very specific to wealth (e.g.~\cite{bouchard}), meaning money and properties, while the simple model here presented is envisioned as way more general, and confluent with potentially any other model for power laws.
...
Also, it is worth to keep in mind that not all models of wealth distribution result in power laws~\cite{dragulescu}, and that here the concept of wealth is wider than that of money and possessions. One may think of knowledge or health as a type of wealth in the following sections.

We argue that power laws, and their inherent extreme inequality, result potentially always from a uniform distribution of the resources, but are at least in agreement with such and uniform distribution.
